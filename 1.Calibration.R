...
ev_metrics_vine_W <- getEvaluation(M_vine_W,stat=c('TSS','Kappa','AUC', 'specificity', 'sensitivity'),opt="max(se+sp)")
mean_ev_metrics_vine_W <- as.data.frame(round(colMeans(x=ev_metrics_vine_W),3))
#
getModelInfo(M_vine_D)
getVarImp(M_vine_D)
plot(getVarImp(M_vine_D),'auc')
rcurve(M_vine_D, id=, mean=TRUE, smooth=TRUE)
ev_metrics_vine_D <- getEvaluation(M_vine_D,stat=c('TSS','Kappa','AUC', 'specificity', 'sensitivity'),opt="max(se+sp)")
mean_ev_metrics_vine_D <- as.data.frame(round(colMeans(x=ev_metrics_vine_D),3))
#
getModelInfo(M_vine_WD)
getVarImp(M_vine_WD)
plot(getVarImp(M_vine_WD),'auc')
rcurve(M_vine_WD, id=, mean=TRUE, smooth=TRUE)
ev_metrics_vine_WD <- getEvaluation(M_vine_WD,stat=c('TSS','Kappa','AUC', 'specificity', 'sensitivity'),opt="max(se+sp)")
mean_ev_metrics_vine_WD <- as.data.frame(round(colMeans(x=ev_metrics_vine_WD),3))
